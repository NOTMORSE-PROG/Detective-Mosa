...
## File naming — keep the existing convention

`{CHARACTER}/{CHARACTER}-{Pose}{Direction}.png`, all-caps folder and filename prefix, e.g.
`MOSA/MOSA-IdleFront.png`. Three different artists converged on this independently across
the first two deliveries — it's canonical now. Don't invent a new scheme.

**Reference sheets are not game assets.** A combined turnaround/expression sheet (no
`Front`/`Left`/`Right` suffix, e.g. `MOSA-Idle.png`, `MOSA-Expressions.png`) is working
...
## Pixel art is the one exception, and it's a per-node setting, not a per-folder one

`MOSA-All-Sprites.png` (Mosa's walk-cycle sheet, `art/characters/mosa/movement/`, 256×256, a
grid of small native-resolution frames) is the **only** pixel art in the game so far.
Godot 4 has **no per-folder import default** — verified against Godot's own docs at `DM-008`
pickup, correcting this ticket's original assumption. Texture filtering (crisp/nearest vs.
smoothed/linear) is a property of whichever node **displays** a texture, not a setting on the
image file itself. This means nothing needs configuring today: the project-wide default
(linear/smoothed) is correct for every painted asset delivered so far, and the walk-cycle
sheet doesn't need `TEXTURE_FILTER_NEAREST` set until `DM-017` actually builds the node that
displays it. Documented here so that ticket doesn't have to rediscover this.

## Deferred — not yet confirmed slots, so not in the manifest below

Two poses are deliberately **not** tracked as slots yet, so don't be alarmed they're
missing from the table: **Mang Ver's expressions beyond Idle** (drawn on his reference
sheet, never individually cropped — only becomes a real slot if `DM-015`'s prologue needs
him to visibly react) and **Aling Vilma's Phones pose** (only a combined reference sheet
exists — currently moot, since the prologue's warning beat is a phone-screenshot UI asset,
not a rendered character). Neither is forgotten; both are one narrative confirmation away
from becoming a real row in `data/asset_manifest.tres`.
...
hand-edit this section, edit the manifest and regenerate. Counts: 37 final, 7
placeholder, 0 missing, 44 total._
...
| Alingvilma | Smile | final | **placeholder** | **placeholder** | (see manifest) |
...
Kap. Torres (normal + deepfaked): no delivered art, only unconfirmed PDF-extract candidates.
Madam Baby: delivered file is an uncropped 3-up reference sheet, not a usable sprite yet.
Aling Mila and Mang Tomas: both fully delivered and directional, same quality bar as the
Ch1 cast, already organized under `art/characters/`. Dali Mart (`bilistore.png`): delivered,
but stylistically mismatched against every other asset (dithered/pixel-retro technique vs.
everyone else's clean cel-shading — a real cross-artist consistency gap for whoever picks up
`DM-032`/M6) and still missing its framing layer. Radyo Kanto: no backdrop at all yet.
